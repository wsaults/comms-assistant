...
def report_channel_messages(messages, client_id, workspace_name):
...
def check_workspace(workspace_name, workspace_config, hours=1, notify=True):
    ...
    if recent_mentions and httpx and notify:
    ...
    if monitored_channels:
        ...
        if all_channel_messages and httpx and notify:
            log(f"\nReporting {len(all_channel_messages)} channel message(s) to monitoring server...")
            report_channel_messages(all_channel_messages, client_id, workspace_display_name)
...
def main():
    ...
    for workspace_name, workspace_config in workspaces.items():
        mentions = check_workspace(workspace_name, workspace_config, hours=args.hours, notify=args.notify)
        if mentions:
